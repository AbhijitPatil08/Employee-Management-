...
-- salary: ₹70,000, DOJ: 2024-01-01) and Aarav Jain (email: 2024-01-02, salary: 
...
('Riya','Patel', 12, 72000, '2024-04-03','riya.patel@example.com');
...
-- Situation -2
--  To organize a corporate event, we need employees who joined after 2024-01-01.Retrieve their details. 
 
select * from employees
where DateOfJoining > '2024-01-01';

-- Situation -3 
-- The finance team is preparing budgets. List all employees whose salary is greater than ₹60,000. 

select * from employees
where salary > 60000;

-- Situation -4 
-- We are sending a company-wide newsletter. Find the details of employees whose email ends with '@example.com'. 
...
where d.departmentname = "Marketing" and e.salary < 60000; 
...
select * from employees
where DATE_FORMAT(DateOfJoining,'%Y-%m') = '2024-02';
...
-- Situation -9 
-- We need to identify senior employees. Retrieve the details of employees who joined before 2024-03-01 and earn more than ₹55,000. 

select * from employees
where DateOfJoining < "2024-03-01" and salary > 55000;

-- Situation -10
--  A special campaign is being planned for team leaders. Find employees who have "Manager" in their name.
SELECT * FROM Employees
...
-- Situation -12
--  The board is interested in high-budget projects. Find the details of departments where the budget exceeds ₹10,00,000. 
select * from departments
...
-- Situation -14 
-- Marketing expansion is being planned. Find all departments whose names start with "M". 
select * from departments
where DepartmentName like 'm%';


-- Situation -15 
-- To allocate funds efficiently, list all departments where the budget is between ₹8,00,000 and ₹15,00,000. 
select * from departments
...
select DepartmentID, count(*) as joined_in_2024 from employees
...
select DepartmentID, count(*) as Number_Employee  from employees
where salary between 50000 and 70000
group by DepartmentID;

-- Situation -10: For diversity analysis, count the number of employees whose names start with each letter of the alphabet.
select left(firstname,1) as Initial, COUNT(*) AS CountByInitial from employees
...
select count(*) as number_of_departments, Location from departments
group by Location;
...
SELECT DepartmentID, CONCAT(DepartmentName, ', ' , Location) AS Department_Address FROM departments;

-- Situation -3: To prepare a personalized email greeting, concatenate "Hello " with the employee's name and a comma.
Select employeeid, CONCAT('Hello ', firstname, ' ', lastname, ',') AS FullName from employees;

-- Situation -4: For creating a unique employee ID, concatenate the department ID and employee number.
select concat('D', departmentid, ' ' , '_e', employeeid) as Unique_ID from employees;
...
select concat(year(DateOfJoining), '-', firstname) from employees;
...
-- Situation -1:
--  As part of an organizational review, we need to update the salary of all employees who are part of the IT department (DepartmentID:3). The salary of all employees in this department should be increased by 10%. Make sure that the increase is applied to every employee within the department.
update employees
...
-- Situation -2: After the recent promotion evaluation, it has been decided that the employee with EmployeeID 41 should be promoted to a "senior Software Engineer" role. Update their job title accordingly and make sure the position is reflected correctly in the employee records.
update employees
...
select EmployeeID, FirstName, LastName, JobTitle, DepartmentID from employees
where DepartmentID = 3;

-- Situation -3: The HR team has informed us of a change in the contact information of employee with EmployeeID 12. The new contact number for this employee should be updated to "9876543210". Ensure that the correct phone number is reflected in the system for any future communications.
update employees
...
-- Situation -5: It has come to our attention that there was an error in the naming of the HR department. The department name for DepartmentID 1 needs to be updated from "HR" to "Human Resources" to reflect the company's official title. Perform this update to ensure consistency across the records.
update departments
...
-- Situation -8: Update the Salary of EmployeeID 21 to ₹60,000 as part of their annual appraisal.
update employees
set salary = 60000
where employeeid = 21;
...
-- Situation -10: The employee with EmployeeID 30 had a delayed start date due to personal reasons. We need to update their joining date to the correct date, which is 2024-03-15, to ensure the records accurately reflect the employee's actual start date with the company.
update employees
...
-- Situation -1: Due to an employee leaving the company, we need to delete the record of EmployeeID 54 from the Employee table.
...
AND (ProjectStatus IS NULL OR ProjectStatus <> 'Active');
...
-- Situation -6: An employee, EmployeeID 46, was added mistakenly and is no longer part of the organization. Please delete their record from the Employee table.
delete from employees
where employeeid = 46;

-- Situation -7: The Marketing department (DepartmentID: 11) has undergone significant layoffs. Delete all records of employees who worked in the Marketing department and joined before 2021.
delete from employees
where DepartmentID = 11 and DateOfJoining < '2021-01-01'; 
...
-- Situation -2: As the company has decided to start tracking employees' marital status, add a newcolumn MaritalStatus of type VARCHAR(20) to the Employee table. 
...
-- Situation -5: To better categorize employees, we need to add a new column EmployeeCategory in the Employee table. The column should accept values like "Full-time", "Part-time", or "Contract". 
...
-- Situation -10: We need to create a foreign key constraint between the Employee table and the Department table to ensure the integrity of the department data linked to each employee.
